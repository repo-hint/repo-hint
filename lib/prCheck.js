...
async function processCodeRule (prInfo, options) {
	...
	for (let rule of rules.codeRules) {
		const processor = new PRProcessor(prInfo, rule);
...
function loadConfiguration (configFolderPath) {
	rules.codeRules = require(path.join(configFolderPath, 'repohint-rules', 'rules-code'));
	rules.criterionRules = require(path.join(configFolderPath, 'repohint-rules', 'rules-criterion'));
	rules.preProcessors = require(path.join(configFolderPath, 'repohint-rules', 'preprocessors'));
