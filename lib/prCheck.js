...
async function prCheck ({number, sha1, author, title, description, files}, configFolderPath, options) {
	...
	if (sha1 && !/origin\/pr/.test(sha1)) { disableComment = true; }
	...
	commentSuffix = config['COMMENT_SUFFIX'] ? config['COMMENT_SUFFIX'] : '';
...
async function processCodeRule (prInfo, options) {
	...
	for (let rule of rules.codeRules) {
		...
		// send comment
		if (comments && !disableComment) {
			api.createPRReview(
				number,
				comments + commentSuffix
			);
		}
	}
...
async function processCriterionRule (prInfo, config, options) {
	...
	for (let rule of rules.criterionRules) {
		...
		if (comments && !disableComment) { commentContent.push(comments); }
	}
	...
	await api.setPRStatus(number, state ? 'success' : 'failure', 'PRInfo Inspection', '', state ? '标题描述符合规范' : '标题描述不符合规范');
}
